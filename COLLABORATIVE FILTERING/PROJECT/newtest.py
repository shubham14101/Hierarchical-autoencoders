...
mapping = tf.placeholder("float", [1213,inputSize]) 
...
X = tf.placeholder("float", [1213,inputSize])
...
for i in range(0,1):
	...
	with tf.Session(config=config) as sess:
		...
		for k in range(0,200):
			print(k)
			losst = 0
			for q in range(0,971):
				#itemInfo = ItemEncoding[q*10681:(q+1)*10681,:]
				#Anm = np.multiply(A,inputLastIND)
				#Am = A - Anm
				opt,lossP,V_1,V_2,V_3,mu_1,mu_2,mu_3,S_1,S_2,S_3,pi_3,pi_2,pi_1= sess.run([optimizer,loss,V1,V2,V3,mu1,mu2,mu3,S1,S2,S3,pi3,pi2,pi1],feed_dict={X:ItemMatrix[q*11:(q+1)*11,:],mapping:ItemCompletion[q*11:(q+1)*11,:]})
				#summary_writer.add_summary(summary,  k*1 + q)
				losst = losst + lossP

			print(losst/971)

		#Y_S1 = np.zeros([71567,10681])
		#Y_S2 = np.zeros([71567,10681])
		#Y_S3 = np.zeros([71567,10681])
		...
		#Y_S1 = np.matmul(sigm(np.matmul(UserMatrix,V_1) + mu_1),S_1) + pi_1
		#Y_S2 = np.matmul(sigm(np.matmul(sigm(np.matmul(UserMatrix,V_1) + mu_1),V_2) + mu_2),S_2) + pi_2
		#Y_S3 = np.matmul(sigm(np.matmul(sigm(np.matmul(sigm(np.matmul(UserMatrix,V_1) + mu_1),V_2) + mu_2),V_3) + mu_3),S_3) + pi_3
		Y_pool_1 = np.zeros([10681,71567])
		for p in range(0,971):
			print(p)
			Y_pool_1[p*11:(p+1)*11,:] = (np.matmul(sigm(np.matmul(ItemMatrix[p*11:(p+1)*11,:],V_1) + mu_1),S_1) + pi_1 +np.matmul(sigm(np.matmul(sigm(np.matmul(ItemMatrix[p*11:(p+1)*11,:],V_1) + mu_1),V_2) + mu_2),S_2) + pi_2+np.matmul(sigm(np.matmul(sigm(np.matmul(sigm(np.matmul(ItemMatrix[p*11:(p+1)*11,:],V_1) + mu_1),V_2) + mu_2),V_3) + mu_3),S_3) + pi_3)/3
		...
		for line in TestingSet.readlines():
			...
			if(x[1]>10681):
				x[1] = round(x[1]/10)
			if(cc2%10==i):
				prediction1 = Y_pool_1[int(x[1])-1,int(x[0])-1]
