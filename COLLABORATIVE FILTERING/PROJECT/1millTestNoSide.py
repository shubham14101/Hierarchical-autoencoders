...
regularize = layers.apply_regularization(layers.l2_regularizer(scale=lambdaR),weights_list=[V1,V2,V3,W3,W1,W2,S1,S2,S3])

differenceLast = X - Ylast
difference1 = X - YS1
difference2 = X - YS2
difference3 = X - YS3


LossLast = tf.reduce_mean(tf.reduce_sum(tf.square(differenceLast), 1, keep_dims=True))
Loss1 = tf.reduce_mean(tf.reduce_sum(tf.square(difference1), 1, keep_dims=True))
Loss2 = tf.reduce_mean(tf.reduce_sum(tf.square(difference2), 1, keep_dims=True))
Loss3 = tf.reduce_mean(tf.reduce_sum(tf.square(difference3), 1, keep_dims=True))



loss = LossLast + Loss1 + Loss2  + Loss3 + regularize
...
for i in range(1,2):
	...
	with tf.Session() as sess:
		...
		Y_pool_1 = (Y_last + Y_S1 +Y_S2+Y_S3)/4
		Y_pool_2 = Y_last
		Y_pool_3 = Y_S1
		...
		for line in TestingSet.readlines():
			...
			if(cc2%10 == i):
				...
				prediction2 = Y_pool_2[x[1]-1,x[0]-1]# - meanDat + itemBias[x[1]-1] + userBias[x[0]-1]
				prediction3 = Y_pool_3[x[1]-1,x[0]-1]# - meanDat + itemBias[x[1]-1] + userBias[x[0]-1]
				print(prediction3,prediction2,prediction1,x[2])
				if(not(math.isnan(prediction1) or math.isnan(prediction2) or math.isnan(prediction3))):
